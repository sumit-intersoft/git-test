...
class ModelToolKaImport extends Model {
	...
	*/
	...
	public function isDBPrepared() {
		...
		$tbl = $prefix . "ka_product_import";
		$res = $this->db->query("DESCRIBE `" . $tbl . "` 'is_new'");
		if (empty($res->rows)) {
			return false;
		}
	...
	*/
	...
	public function formatDateTime(&$str) {
		...
		if (!$this->formatDate($pair[0])) {
			return false;
		}
		
		if (empty($pair[1]) || !$this->formatTime($pair[1])) {
			$pair[1] = '00:00:00';
		}
	...
	*/
	protected function updateProduct($product_id, $data, $is_new) {
		...
		if (!empty($data['date_added'])) {
			if (!$this->formatDateTime($data['date_added'])) {
				$this->addImportMessage($this->product_mark . "Wrong date format in 'date added' field.");
			} else {
				if ($data['date_added'] != '0000-00-00 00:00:00') {
		...
		if (!empty($data['date_modified'])) {
			if (!$this->formatDateTime($data['date_modified'])) {
				$this->addImportMessage($this->product_mark . "Wrong date format in 'date modified' field.");
			} else {
	...
	public function getFieldSets() {
		...
		$fields2 = array(			
			...
			array(
				'field' => 'date_added',
				'name'  => 'Date Added',
				'descr' => "Format: 'YYYY-MM-DD' or 'YYYY-MM-DD HH:MM:SS', Example: 2012-03-25 20:44:00"
			),
			array(
				'field' => 'date_modified',
				'name'  => 'Date Modified',
